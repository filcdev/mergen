 ...
 * sign-in survives the app being killed while the user is authenticating.
 */
internal class EntraAuthFlow(
    ...
    /** Runs the interactive sign-in and returns an ID-token proof for Chronos. */
    suspend fun acquireIdToken(): IdTokenProof {
        ensureConfigured()
        val flow = oidc.newPendingFlow()
        storage.writePendingFlow(flow)
        val redirectUri =
            webAuthSession.authorize(
                oidc.authorizationUrl(flow),
                AuthConfig.CALLBACK_SCHEME,
            )
        return completeSignIn(redirectUri)
    }

    /** Completes a sign-in whose browser round-trip outlived this app process. */
    suspend fun completeSignIn(redirectUri: String): IdTokenProof {
        ensureConfigured()
        val flow =
        ...
        val code =
            redirect.code
                ?: throw AuthException("The sign-in response contained no authorization code.")
    ...
    /**
     * ID token from the stored refresh token, or `null` when there is none.
     * A refresh-token grant is not a new authorization request, so it has no new OIDC nonce.
     */
    suspend fun refreshIdToken(): IdTokenProof? {
        ...
        return IdTokenProof(token = tokens.idToken)
    }
    ...
    private fun ensureConfigured() {
        if (!AuthConfig.isConfigured) {
            throw AuthException(
                "This build has no Entra configuration. Set AuthConfig.ENTRA_CLIENT_ID and ENTRA_TENANT_ID.",
            )
        }
    }
